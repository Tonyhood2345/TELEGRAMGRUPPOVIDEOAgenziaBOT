...
def create_wp_listing(title, content, featured_media_id, images_urls, video_url):
    ...
    mortgage_summary_html = (
        f'\n<div class="property-mortgage-summary" style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:16px; padding:20px; margin-bottom:25px; font-family:\'Outfit\', sans-serif; display:flex; flex-wrap:wrap; justify-content:space-between; align-items:center; gap:15px;">\n'
        f'  <div>\n'
        f'    <span style="color:#64748b; font-size:12px; font-weight:700; text-transform:uppercase;">Valore Immobile</span>\n'
        f'    <strong style="font-size:24px; color:#0f172a; display:block;">{valore_immobile_formatted} €</strong>\n'
        f'  </div>\n'
        f'  <div style="background:#ffffff; padding:10px 20px; border-radius:12px; border:1px solid #cbd5e1; text-align:right; min-width:180px;">\n'
        f'    <span style="color:#10b981; font-size:11px; font-weight:700; text-transform:uppercase; display:block;">Rata Mutuo (80%)</span>\n'
        f'    <strong style="font-size:22px; color:#10b981;">~ {rata_mensile_formatted} € <span style="font-size:12px; font-weight:normal; color:#64748b;">/ mese</span></strong>\n'
        f'  </div>\n'
        f'</div>\n'
    )
    ...
    # === SEZIONE 9: FORM RICHIESTA MUTUO (ACCORDION AL CLICK SULLA RATA) ===
    ...
    mortgage_calculator_html = (
        ...
        f'  \n'
        f'  <!-- Card Rata interattiva come bottone accordion -->\n'
        f'  <div id="mortgage-trigger-card" onclick="toggleMortgageForm()" style="background:linear-gradient(135deg, #ffffff, #f1f5f9); border:2px solid #10b981; border-radius:12px; padding:18px; text-align:center; cursor:pointer; transition:all 0.3s ease; box-shadow:0 4px 10px rgba(16,185,129,0.08);">\n'
        f'    <span style="color:#64748b; font-size:12px; display:block; font-weight:700; text-transform:uppercase; letter-spacing:0.5px;">RATA MENSILE STIMATA (LTV 80%)</span>\n'
        f'    <strong style="font-size:34px; color:#10b981; display:block; margin:5px 0;">{rata_mensile_formatted} € <span style="font-size:14px; font-weight:normal; color:#64748b;">/ mese</span></strong>\n'
        f'    <span style="font-size:12px; color:#64748b; display:block; line-height:1.4; margin-bottom:8px;">{nota_mutuo}</span>\n'
    ...
    contact_box_html = (
        ...
        f'  \n'
        f'  <script>\n'
        f'    // Generatore Calendario Settimana Corrente con slot quasi sempre impegnati\n'
        ...
        f'      let now = new Date();\n'
        f'      \n'
        f'      // Genera i prossimi 6 giorni lavorativi (esclusa Domenica)\n'
        f'      let count = 0;\n'
        f'      let dayIndex = 0;\n'
        f'      while (count < 6) {{\n'
        ...
        f'        dayIndex++;\n'
        f'        \n'
        f'        if (futureDate.getDay() === 0) continue; // Salta domenica\n'
        f'        \n'
        ...
        f'        \n'
        f'        // Genera gli orari per ciascun giorno\n'
        f'        hours.forEach((h, idx) => {{\n'
        f'          // Algoritmo deterministico per creare urgenza: quasi tutti impegnati (solo 1 o 2 per giorno disponibili)\n'
    ...
    daria_sticky_widget_html = (
        ...
        f'  </style>\n'
        f'  \n'
        f'  <!-- 1. FUMETTO NOTIFICA TEMPORIZZATO (SPEECH BUBBLE) -->\n'
        ...
        f'  </div>\n'
        f'  \n'
        f'  <!-- 2. IL BOTTONE ROTONDO FLUTTUANTE (AVATAR) -->\n'
        ...
        f'  </div>\n'
        f'  \n'
        f'  <!-- 3. IL CASSETTO DEI CONTATTI ESPANSO (DRAWER) -->\n'
        ...
        f'\n'
        f'<script>\n'
        f'  // 1. Apparizione e scomparsa temporizzata del fumetto notifica\n'
        ...
        f'    }}, 50);\n'
        f'    \n'
        f'    // Nasconde automaticamente il fumetto dopo 6 secondi\n'
        f'    setTimeout(function() {{\n'
        f'      closeDariaBubble();\n'
        f'    }}, 6000);\n'
        f'  }}, 3000);\n'
        ...
        f'  }}\n'
        f'  \n'
        f'  // 2. Apertura e Chiusura Cassetto DarIA\n'
