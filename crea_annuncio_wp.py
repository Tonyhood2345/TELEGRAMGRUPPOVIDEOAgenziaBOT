...
def create_wp_listing(title, content, featured_media_id, images_urls, video_url):
    ...
    ape_badge_html = (
        f'\n<div class="ape-badge-wrapper" onclick="triggerDariaApeExplanation()" title="Clicca per spiegazione DarIA" style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:16px; padding:20px; margin-bottom:25px; font-family:\'Outfit\', sans-serif; cursor:pointer;">\n'
        f'  <style>\n'
    ...
    mortgage_calculator_html = (
        ...
        f'  <div id="mortgage-trigger-card" onclick="toggleMortgageForm(); triggerDariaMortgageExplanation();" style="background:linear-gradient(135deg, #ffffff, #f1f5f9); border:2px solid #10b981; border-radius:12px; padding:18px; cursor:pointer; transition:all 0.3s ease; box-shadow:0 4px 10px rgba(16,185,129,0.08);">\n'
    ...
    daria_sticky_widget_html = (
        ...
        f'  </div>\n'
        f'  \n'
        f'  <!-- 2. BOTTONE AVATAR (CON IMMAGINE CASETTA PERSONALIZZATA DARIA) -->\n'
        f'  <div id="daria-button-collapsed" class="daria-round-trigger" onclick="openDariaDrawer()">\n'
        f'    <div class="daria-trigger-pulse"></div>\n'
        f'    <img src="https://www.immobiliaregiancani.it/wp-content/uploads/2026/07/daria-avatar.png" style="width:54px; height:54px; object-fit:contain; position:relative; z-index:2;" />\n'
        ...
        f'      <div style="position:relative; width:40px; height:40px; background:#ffffff; border-radius:50%; display:flex; align-items:center; justify-content:center; padding:3px; overflow:hidden;">\n'
        f'        <img src="https://www.immobiliaregiancani.it/wp-content/uploads/2026/07/daria-avatar.png" style="width:100%; height:100%; object-fit:contain;" />\n'
        ...
        f'  }}\n'
        f'  \n'
        f'  /* Triggers APE e Mutuo da DarIA */\n'
        f'  const classApe = "{ape_class}";\n'
        f'  const rataMensile = "{rata_mensile_formatted if has_declared_price else "300"}";\n'
        f'  window.triggerDariaApeExplanation = function() {{\n'
        f'    openDariaDrawer();\n'
        f'    const container = document.getElementById("daria-chat-messages");\n'
        f'    container.innerHTML = "";\n'
        f'    appendChatMessage("L\'APE è l\'Attestato di Prestazione Energetica che misura l\'efficienza energetica dell\'immobile. Per questa casa la classe energetica è " + classApe + ". Antonio Giancani si mette a tua completa disposizione per illustrarti i dettagli tecnici dei consumi o mostrarti le bollette!", "msg-daria");\n'
        f'  }};\n'
        f'  window.triggerDariaMortgageExplanation = function() {{\n'
        f'    openDariaDrawer();\n'
        f'    const container = document.getElementById("daria-chat-messages");\n'
        f'    container.innerHTML = "";\n'
        f'    appendChatMessage("Comprare questa casa con un mutuo all\'80% è un\'ottima opportunità! Con una rata stimata a partire da circa " + rataMensile + " € al mese, l\'acquisto è conveniente. Antonio Giancani mette a tua completa disposizione broker creditizi convenzionati per una consulenza gratuita!", "msg-daria");\n'
        f'  }};\n'
        f'  \n'
        f'  /* Cookie e tracciamento click/dwell time delle sezioni */\n'
        f'  (function() {{\n'
        f'    const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzq3gfy5JCZJT1tyF4oECkBFVrckxVzqDuAJgUSvPhU4rv2Bztj7EUT3m4b5ILm4Vdc/exec";\n'
        f'    const title = "{js_safe_title}";\n'
        f'    let visitorId = localStorage.getItem("daria_visitor_id");\n'
        f'    if (!visitorId) {{\n'
        f'      visitorId = "VIS-" + Math.random().toString(36).substring(2, 11).toUpperCase() + "-" + Date.now();\n'
        f'      localStorage.setItem("daria_visitor_id", visitorId);\n'
        f'    }}\n'
        f'    function sendLog(data) {{\n'
        f'      const payload = {{\n'
        f'        action: "log_tracking",\n'
        f'        visitor_id: visitorId,\n'
        f'        immobile: title,\n'
        f'        device: window.innerWidth < 768 ? "Mobile" : "Desktop",\n'
        f'        resolution: window.innerWidth + "x" + window.innerHeight,\n'
        f'        referrer: document.referrer || "Accesso Diretto",\n'
        f'        ...data\n'
        f'      }};\n'
        f'      fetch(APPS_SCRIPT_URL + "?action=log_tracking&data=" + encodeURIComponent(JSON.stringify(payload)), {{ mode: "no-cors" }}).catch(() => {{}});\n'
        f'    }}\n'
        f'    sendLog({{ event: "page_view" }});\n'
        f'    document.addEventListener("click", function(e) {{\n'
        f'      let target = e.target;\n'
        f'      while (target && target !== document) {{\n'
        f'        if (target.id === "daria-bubble" || target.id === "daria-button-collapsed") {{\n'
        f'          sendLog({{ event: "click", element: "DarIA Widget Trigger" }});\n'
        f'          break;\n'
        f'        }}\n'
        f'        if (target.classList && target.classList.contains("ape-badge-wrapper")) {{\n'
        f'          sendLog({{ event: "click", element: "APE Badge Section" }});\n'
        f'          break;\n'
        f'        }}\n'
        f'        if (target.id === "mortgage-trigger-card") {{\n'
        f'          sendLog({{ event: "click", element: "Mutuo Accordion Card" }});\n'
        f'          break;\n'
        f'        }}\n'
        f'        if (target.tagName === "A" && target.href) {{\n'
        f'          if (target.href.includes("wa.me") || target.href.includes("whatsapp")) {{\n'
        f'            sendLog({{ event: "click", element: "WhatsApp Contact (" + target.innerText.trim() + ")" }});\n'
        f'          }} else if (target.href.startsWith("tel:")) {{\n'
        f'            sendLog({{ event: "click", element: "Phone Call Contact" }});\n'
        f'          }} else if (target.href.includes("youtube.com") || target.href.includes("facebook.com") || target.href.includes("instagram.com") || target.href.includes("threads.net") || target.href.includes("tiktok.com")) {{\n'
        f'            sendLog({{ event: "click", element: "Social Link: " + target.href }});\n'
        f'          }} else {{\n'
        f'            sendLog({{ event: "click", element: "External Link: " + target.href }});\n'
        f'          }}\n'
        f'          break;\n'
        f'        }}\n'
        f'        if (target.tagName === "BUTTON" || target.type === "submit") {{\n'
        f'          sendLog({{ event: "click", element: "Form Button (" + target.innerText.trim() + ")" }});\n'
        f'          break;\n'
        f'        }}\n'
        f'        target = target.parentNode;\n'
        f'      }}\n'
        f'    }});\n'
        f'    const trackedSections = [\n'
        f'      {{ id: "property-gallery", selector: ".property-gallery-grid" }},\n'
        f'      {{ id: "property-desc", selector: ".property-description" }},\n'
        f'      {{ id: "property-video", selector: ".property-video-container" }},\n'
        f'      {{ id: "property-ape", selector: ".ape-badge-wrapper" }},\n'
        f'      {{ id: "property-map", selector: ".property-map-section" }},\n'
        f'      {{ id: "property-mortgage", selector: ".property-mortgage-calculator" }},\n'
        f'      {{ id: "property-booking", selector: ".property-contact-box" }}\n'
        f'    ];\n'
        f'    const activeObservations = {{}};\n'
        f'    if (typeof IntersectionObserver !== "undefined") {{\n'
        f'      const observer = new IntersectionObserver((entries) => {{\n'
        f'        entries.forEach(entry => {{\n'
        f'          const id = entry.target.dataset.trackId;\n'
        f'          if (entry.isIntersecting) {{\n'
        f'            activeObservations[id] = Date.now();\n'
        f'          }} else {{\n'
        f'            if (activeObservations[id]) {{\n'
        f'              const duration = Math.round((Date.now() - activeObservations[id]) / 1000);\n'
        f'              if (duration > 0) {{\n'
        f'                sendLog({{ event: "dwell_time", section: id, seconds: duration }});\n'
        f'              }}\n'
        f'              delete activeObservations[id];\n'
        f'            }}\n'
        f'          }}\n'
        f'        }});\n'
        f'      }}, {{ threshold: 0.3 }});\n'
        f'      trackedSections.forEach(sec => {{\n'
        f'        const el = document.querySelector(sec.selector);\n'
        f'        if (el) {{\n'
        f'          el.dataset.trackId = sec.id;\n'
        f'          observer.observe(el);\n'
        f'        }}\n'
        f'      }});\n'
        f'      window.addEventListener("beforeunload", function() {{\n'
        f'        for (const id in activeObservations) {{\n'
        f'          const duration = Math.round((Date.now() - activeObservations[id]) / 1000);\n'
        f'          if (duration > 0) {{\n'
        f'            sendLog({{ event: "dwell_time_final", section: id, seconds: duration }});\n'
        f'          }}\n'
        f'        }}\n'
        f'      }});\n'
        f'    }}\n'
