...
def main():
    try:
        ...
        # Mappatura delle colonne basata sulla struttura di DATABASE_IMMOBILI
        idx_desc = headers.index("DESCRIZIONE") if "DESCRIZIONE" in headers else 4
        
        # Cerchiamo prima LINK_YOUTUBE, altrimenti LINK, altrimenti LINK_FACEBOOK
        idx_video = -1
        ...
        print(f"=== ESECUZIONE PIPELINE GITHUB ACTIONS PER I PRIMI 2 IMMOBILI (DATABASE_IMMOBILI) ===")
        processed_count = 0
        ...
        for idx, row in enumerate(rows, start=2):
            ...
            # Filtro opzionale se la colonna PUBBLICATO è presente
            is_published = True
            if idx_pub != -1 and idx_pub < r_len:
                is_published = (row[idx_pub].strip().upper() == "SI")
                
            # Se la riga ha sia descrizione che video link, ed è pubblicata, la elaboriamo
            if desc_text and video_url and video_url.startswith("http") and is_published:
                ...
                clean_title = re.sub(r'\b\d{4}[-/]\d.py\b', '', first_line) # pulizia date generiche
                clean_title = re.sub(r'\b\d{2}[-/]\d{2}[-/]\d{4}\b', '', clean_title)
                clean_title = re.sub(r'\s*-\s*$', '', clean_title.strip())
                clean_title = re.sub(r'^\s*-\s*', '', clean_title)
                clean_title = clean_title.strip()
                
                # Accorciamo se troppo lungo per il titolo
                if len(clean_title) > 80:
                    clean_title = clean_title[:77] + "..."
                    
                print(f"\n--- [{processed_count}/2] Elaborazione riga {idx}: {clean_title} ---")
                ...
                if res.returncode == 0:
                    ...
                    # Stampiamo l'output per far vedere il link dell'immobile creato
                    output_lines = res.stdout.strip().split('\n')
                    for line in output_lines:
                        if "WordPress creato" in line or "Easy MCP AI" in line or "http" in line:
                            print(f"    {line}")
                else:
                    print(f"  ❌ Errore durante l'esecuzione per la riga {idx}.")
                    print(f"  Logs Errore:\n{res.stderr}")
                
                # Ci fermiamo dopo i primi 2 immobili come richiesto dall'utente
                if processed_count >= 2:
                    print("\n🛑 Raggiunto il limite di 2 immobili di test richiesto dall'utente. Termino la prova.")
                    break
                    
        print(f"\n🎉 PIPELINE COMPLETATA! Elaborati {processed_count} immobili di test.")
